...
def edit_profile(request, username):
    if request.method == 'POST':
        ...
        if user.email != request.POST['email']:
            ...
            updated = True  
        if 'password' in request.POST and request.POST['password'] and not check_password(request.POST['password'], user.password):
            user.password = make_password(request.POST['password'])  
            updated = True   
        if 'profile_pic' in request.FILES and str(user.profile_pic) != str(request.FILES['profile_pic']): 
            user.profile_pic = request.FILES['profile_pic']
            updated = True
        if 'cover' in request.FILES and str(user.cover) != str(request.FILES['cover']): 
        ...
        return render(request, "network/editprofile.html", context)
